...
export const Dashboard = () => {
  const PAGE_NUMBER_DEFAULT: number = Number(
    import.meta.env.VITE_DEFAULT_PAGE_NUMBER,
  )
  const PAGE_SIZE_DEFAULT: number = Number(
  ...
  useEffect(() => {
    ;(async () => {
      const result: PayloadData = await fetchSalesByPageSizeAndNumber(
        pageSize,
        pageNumber,
      )
      setSales(result)
  ...
  return (
    <div className="flex justify-center m-4">
      <Tabs
        ...
        className="flex h-fit bg-sky-200 rounded shadow-lg shadow-gray-400/60"
      >
        ...
        <TabsContent className="grid m-2 space-y-4" value="analytics">
          <CalendarDateRangePicker
            className="grid grid-cols-1 justify-self-end bg-sky-50 rouded"
            setSales={setSales}
          />
          <AnalyticsCardsContainer summary={sales?.summary} />
          <TransactionsTypesCard
            items={sales?.items}
            width={width || innerWidth / 4}
          />
